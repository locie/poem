...
transformed parameters {
  ...
  epsilon[1] = y[1] - (f[1] + rho*f[1]);
  for (n in 2:N_train) {
    epsilon[n] = y[n] - (f[n] + rho*y[n-1] + theta*epsilon[n-1]);
  }
...
generated quantities {
  ...
  prediction[1] = normal_rng(f[1] + rho*f[1], sigma);
  log_lik[1] = normal_lpdf(y[1] | f[1] + rho*f[1], sigma);
  for (n in 2:N_train) {
    prediction[n] = normal_rng(f[n] + rho*y[n-1] + theta*epsilon[n-1], sigma);
    log_lik[n] = normal_lpdf(y[n] | f[n] + rho*y[n-1] + theta*epsilon[n-1], sigma);
  }
  prediction[N_train+1] = normal_rng(f[N_train] + rho*y[N_train] + theta*epsilon[N_train], sigma);
  for (m in N_train+2:N) {
    prediction[m] = normal_rng(power_mean(x[m], a1, a2, x1, y1) + rho*prediction[m-1], sigma * sqrt( 1+(rho+theta)^2*(1-rho^(2*(m-N_train-1))) / (1-rho^2) ));
  }
}